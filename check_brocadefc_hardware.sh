...
# =========================================================================================
# 17.03.2025: add fixed snmpv3 command and fix perfdata
...
print_usage() {
        ...
        echo ""
        echo "Usage: $PROGNAME [-H | --hostname HOSTNAME] | [-h | --help] | [-v | --version]"
        echo ""
...
// add fixed snmp connect, change snmp_user, pass1 and pass2, key1 and key2 if needed
COMMAND="snmpwalk -v 3 -u snmp_user -l authPriv -a SHA-512 -A pass1 -x AES-256 -X pass2"

TYPE=$($COMMAND $HOSTNAME .1.3.6.1.2.1.47.1.1.1.1.2.1 | sed "s/.*STRING:\(.*\)$/\1/")

if [ ! "$?" -eq 0 ]; then
    echo "$SENSOR_VALUE UNKNOWN - Could not connect to SNMP server $hostname.";
    exit $STATE_UNKNOWN;
fi

NBR_INDEX=$($COMMAND $HOSTNAME .1.3.6.1.4.1.1588.2.1.1.1.1.22.1.1 | wc -l )
i=1

while [ $i -le $NBR_INDEX ]; do
        SENSOR_VALUE=$($COMMAND $HOSTNAME .1.3.6.1.4.1.1588.2.1.1.1.1.22.1.4.$i | sed "s/.*INTEGER:\(.*\)$/\1/"| sed "s/ //g")
        SENSOR_STATUS=$($COMMAND $HOSTNAME .1.3.6.1.4.1.1588.2.1.1.1.1.22.1.3.$i | sed "s/.*INTEGER:\(.*\)$/\1/")
        SENSOR_INFO=$($COMMAND $HOSTNAME .1.3.6.1.4.1.1588.2.1.1.1.1.22.1.5.$i | sed "s/.*STRING:\(.*\)$/\1/" | sed "s/\"/\:/g" | sed "s/\://g"| sed "s/ //g" | sed "s/#//g" | sed -e "s/\(.*\)/\L\1/")
        SENSOR_TYPE=$($COMMAND $HOSTNAME .1.3.6.1.4.1.1588.2.1.1.1.1.22.1.2.$i | sed "s/.*INTEGER:\(.*\)$/\1/")
        ...
        if [ $SENSOR_VALUE -gt 0 ]; then
          case `echo $SENSOR_STATUS` in
                1) array=( ${array[@]} ${SENSOR_INFO}=${SENSOR_VALUE}${SENSOR_TYPE}, )
                   perfdata=( ${perfdata[@]} ${SENSOR_INFO}=${SENSOR_VALUE}${SENSOR_TYPE}";;;;; " )
                ;;
